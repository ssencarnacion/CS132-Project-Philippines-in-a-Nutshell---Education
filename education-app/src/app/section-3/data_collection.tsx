...
const Button_1 = ({ label, link }: { label: string, link: string }) => {
  return (
    <Link href={link}>
      <button
        className="p-2 bg-blue-700 text-white rounded cursor-pointer mx-1 text-sm hover:bg-indigo-800 w-37 h-10 text-sm" // Adjusted width and height
      >
        {label}
      </button>
    </Link>
...
const Button_2 = ({ label, link }: { label: string, link: string }) => {
  return (
    <Link href={link}>
      <button
        className="p-2 bg-blue-700 text-white rounded cursor-pointer mx-1 text-sm hover:bg-indigo-800 w-480 h-10 text-sm" // Adjusted width and height
      >
        {label}
      </button>
    </Link>
